# Notice Extraction - Project Overview

## 1. Pipeline Architecture
The **Notice Extraction** engine is a modular Python application designed to process government tax notices and extract strict structured business fields from them. 

The pipeline works in a two-phase architecture:
1. **Phase 1 (OCR & Layout Reconstruction):** The application takes an input PDF or image and uses **PaddleOCR** to extract the raw text and spatial 2D coordinates. It then reconstructs the layout to preserve column structure and spacing, outputting a layout-preserved text file.
2. **Phase 2 (Structured AI Extraction):** The layout-preserved text is sent to the **OpenAI API** (usually `gpt-4o` or `gpt-4o-mini`). The API is constrained by strict rules and a Pydantic schema validation to return a precisely structured JSON containing 16 specific fields.

## 2. Extraction Prompt
The extraction heavily relies on a detailed system prompt. Below is the system prompt used in the application:
